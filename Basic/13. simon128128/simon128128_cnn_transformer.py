"""
CNN + Transformer Basic Related-Key Differential Neural Distinguisher for Simon128/128
...
──────────────────────────
  δK forced to δP (0,dp_r). Bias score = per‑output‑bit bias (Definition 4).
...
_BASE = int(0xFFFFFFFFFFFFFFFC)
const_simon128128 = [
    int((_BASE ^ _z0[i % 62]) & MASK_VAL) for i in range(66)
]
assert len(const_simon128128) == 66


def rol64(x: np.ndarray, k: int) -> np.ndarray:
    k = int(k)
    return ((x << np.uint64(k)) | (x >> np.uint64(64 - k))) & _MASK64
...
def expand_key_simon(k, t):
    """Key schedule for Simon128/128 (m=2). k: shape (2, n_samples)."""
    if t < 2:
    ...
    for i in range(t - 2):
        tmp = ror64(ks[i + 1], 3)
        c   = np.uint64(const_simon128128[i])
        ks[i + 2] = (c ^ ks[i] ^ tmp ^ ror64(tmp, 1)) & _MASK64
    return ks
...
def _build_one_diff_samples_basic(n, nr, diff, key_diff, s_groups=8, extra_words=2):
    ...
    """
    keys = np.frombuffer(urandom(8 * 2 * n), dtype=np.uint64).reshape(2, n).copy()
    keys_diff = np.array([
...
def _build_random_negative_samples(n, nr, s_groups=8, extra_words=2):
    ...
    Simon128/128 has m=2 -- two 64-bit key words.
    """
    # Two independent random keys (each is a 2-word vector) and two
    # independent random plaintexts.
    keys_a = np.frombuffer(urandom(8 * 2 * n), dtype=np.uint64).reshape(2, n).copy()
    keys_b = np.frombuffer(urandom(8 * 2 * n), dtype=np.uint64).reshape(2, n).copy()
...
def make_train_data_basic(n, nr, diff=(0x0, 0x2),
                          key_diff=(0x0, 0x2),
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    δK forced equal to δP across both key words: (0,dp_r).
    """
    dp_l = 0
    ...
    dk = (0, dp_r)   # 2‑tuple for Simon128/128 (m=2)

    keys = np.frombuffer(urandom(8 * 2 * n_samples), dtype=np.uint64).reshape(2, n_samples).copy()
    keys_d = np.array([
...
def _generate_initial_population(L_squared, plain_bits, min_hw, max_hw, rng):
    ...
    while len(population) < L_squared and attempts < max_attempts:
        attempts += 1
        # Generate 64-bit random integer without overflow using bytes
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simon128_128_Basic_v2')
...
# ============================================================================
# Table 3 defaults (basic distinguisher) for Simon128/128 – used as fallback
# ============================================================================

TABLE3_DIFFS = [
    {"nr": 13, "diff": (0x0, 0x2), "key_diff": (0x0, 0x2),
     "expected_acc": 0.9956, "expected_tpr": 0.9949, "expected_tnr": 0.9963},
    {"nr": 14, "diff": (0x0, 0x2), "key_diff": (0x0, 0x2),
     "expected_acc": 0.8389, "expected_tpr": 0.8160, "expected_tnr": 0.8616},
    {"nr": 15, "diff": (0x0, 0x2), "key_diff": (0x0, 0x2),
     "expected_acc": 0.6148, "expected_tpr": 0.6009, "expected_tnr": 0.6286},
]
...
def train_basic_distinguisher(nr,
                              ...
                              se_ratio=4,
                              output_dir='./results_simon128128'):
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simon128/128  Basic CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simon128/128 Basic CNN+Transformer v2 -- {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simon128/128 Basic  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simon128/128 Basic  {nr}r  MSE Loss')
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE (Basic)  --  Simon128/128  --  {nr} rounds")
...
def check_simon128128():
    ...
    # check 1: encrypt/decrypt round-trip (68 rounds)
    ks68 = expand_key_simon(keys, 68)
    c0l, c0r = encrypt_simon((p0l, p0r), ks68)
    ...
    print("  check 1 PASSED: encrypt/decrypt round-trip (200 samples, 68 rounds, m=2)")
    ...
    for t in [13, 14, 15]:
        ...
        assert len(kst) == t
    print("  check 3 PASSED: key schedules valid for 13r / 14r / 15r")
    ...
    ks13  = expand_key_simon(keys,  13)
    ks13d = expand_key_simon(keys2, 13)
    c0l13, c0r13 = encrypt_simon((p0l, p0r), ks13)
    c1l13, c1r13 = encrypt_simon((p0l ^ diff[0], p0r ^ diff[1]), ks13d)
    dL = c0l13 ^ c1l13
    dR = c0r13 ^ c1r13
    print(f"  check 4 PASSED: 13r diff (0x0,0x2)  "
          f"sample dL={hex(int(dL[0]))}  dR={hex(int(dR[0]))}")

    assert len(const_simon128128) == 66
    assert all((c & MASK_VAL) == c for c in const_simon128128)
    print("  check 5 PASSED: 66 x 64-bit z0 constants (Simon128/128)")

    _z2_first8 = [1, 0, 1, 0, 1, 1, 1, 1]
    assert _z0[:8] != _z2_first8, "z0 should differ from z2!"
    print("  check 6 PASSED: z0 sequence correctly differs from z2 (Simon128/192)")

    ks3 = expand_key_simon(keys, 3)
    tmp = ror64(ks3[1], 3)
    expected_ks2 = np.uint64(const_simon128128[0]) ^ ks3[0] ^ tmp ^ ror64(tmp, 1)
    assert np.all(ks3[2] == expected_ks2), "m=2 key recurrence mismatch!"
    print("  check 7 PASSED: m=2 key recurrence formula verified")
    print("  Simon128/128 cipher checks PASSED")


def quick_data_check(nr=14, n=100, extra_words=2):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Basic Related-Key Neural Distinguisher for Simon128/128 with GA + Pipeline")
    parser.add_argument('--rounds', type=int, nargs='+', default=[13, 14, 15],
                        help='Round number(s). Default: 13 14 15')
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simon128128')
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  Basic CNN+Transformer Distinguisher  Simon128/128")
