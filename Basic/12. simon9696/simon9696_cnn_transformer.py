"""
CNN + Transformer Basic Related-Key Differential Neural Distinguisher for Simon96/96
...
SIMON96/96 SPECIFICS
────────────────────
  Block  = 96 bits (two 48‑bit half‑words L, R)
  Key    = 96 bits (TWO 48‑bit words k[0..1])   m=2
  Rounds = 52
...
# z0 sequence, period 62 – Simon96/96 uses j=0 (m=2)
...
# 50 round constants: 52 rounds - 2 initial key words = 50 iterations.
_BASE = 0xFFFFFFFFFFFC
const_simon9696 = [(_BASE ^ _z0[i % 62]) & MASK_PY for i in range(50)]
assert len(const_simon9696) == 50
...
def expand_key_simon(k, t):
    """Key schedule for Simon96/96 (m=2). k: shape (2, n_samples)."""
    if t < 2:
    ...
    for i in range(t - 2):
        tmp = ror(ks[i + 1], 3)
        c   = np.uint64(const_simon9696[i])
        ks[i + 2] = (c ^ ks[i] ^ tmp ^ ror(tmp, 1)) & MASK_VAL
    return ks
...
def _build_one_diff_samples_basic(n, nr, diff, key_diff, s_groups=8, extra_words=2):
    ...
    """
    keys = np.zeros((2, n), dtype=np.uint64)
    for w in range(2):
        keys[w] = _rand48(n)
    keys_diff = np.array([
...
def _build_random_negative_samples(n, nr, s_groups=8, extra_words=2):
    ...
    Simon96/96 has m=2 -- two 48-bit key words.
    """
    # Two independent random keys (each is a 2-word vector) and two
    # independent random plaintexts.
    keys_a = np.zeros((2, n), dtype=np.uint64)
    keys_b = np.zeros((2, n), dtype=np.uint64)
    for w in range(2):
        keys_a[w] = _rand48(n)
        keys_b[w] = _rand48(n)
...
def make_train_data_basic(n, nr, diff=(0x0, 0x20),
                          key_diff=(0x0, 0x20),
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    dk = (0, dp_r)   # 2‑tuple for Simon96/96 (m=2)

    keys = np.zeros((2, n_samples), dtype=np.uint64)
    for w in range(2):
...
def _generate_initial_population(L_squared, plain_bits, min_hw, max_hw, rng):
    ...
    while len(population) < L_squared and attempts < max_attempts:
        attempts += 1
        # 48-bit random integer from bytes works and stays within Python int
        dp_r = int.from_bytes(rng.bytes(6), 'little') & MASK_PY
        if dp_r == 0:
            continue
        hw = bin(dp_r).count('1')
        if min_hw <= hw <= max_hw:
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simon96_96_Basic_v2')
...
# ============================================================================
# Table 3 defaults (basic distinguisher) for Simon96/96 – used as fallback
# ============================================================================

TABLE3_DIFFS = [
    {"nr": 11, "diff": (0x0, 0x20), "key_diff": (0x0, 0x20),
     "expected_acc": 0.9999, "expected_tpr": 0.9999, "expected_tnr": 0.9998},
    {"nr": 12, "diff": (0x0, 0x20), "key_diff": (0x0, 0x20),
     "expected_acc": 0.9709, "expected_tpr": 0.9673, "expected_tnr": 0.9745},
    {"nr": 13, "diff": (0x0, 0x20), "key_diff": (0x0, 0x20),
     "expected_acc": 0.6753, "expected_tpr": 0.6771, "expected_tnr": 0.6735},
]
...
def train_basic_distinguisher(nr,
                              ...
                              se_ratio=4,
                              output_dir='./results_simon9696'):
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simon96/96  Basic CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simon96/96 Basic CNN+Transformer v2 -- {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simon96/96 Basic  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simon96/96 Basic  {nr}r  MSE Loss')
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE (Basic)  --  Simon96/96  --  {nr} rounds")
...
def check_simon9696():
    ...
    print("  check 1 PASSED: encrypt/decrypt round-trip (200 samples, 52 rounds, m=2)")
    ...
    c1l11, c1r11 = encrypt_simon(
        (p0l ^ diff[0], p0r ^ diff[1]), ks11d)
    dL = (c0l11 ^ c1l11) & MASK_VAL
    dR = (c0r11 ^ c1r11) & MASK_VAL
    print(f"  check 4 PASSED: 11r diff (0x0,0x20)  "
          f"sample dL={hex(int(dL[0]))}  dR={hex(int(dR[0]))}")

    assert len(const_simon9696) == 50
    assert all((c & MASK_VAL) == c for c in const_simon9696)
    print("  check 5 PASSED: 50 x 48-bit z0 constants (Simon96/96)")

    _z2_first8 = [1, 0, 1, 0, 1, 1, 1, 1]
    assert _z0[:8] != _z2_first8, "z0 should differ from z2!"
    print("  check 6 PASSED: z0 sequence correctly differs from z2 (Simon96/144)")
    print("  Simon96/96 cipher checks PASSED")


def quick_data_check(nr=12, n=100, extra_words=2):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Basic Related-Key Neural Distinguisher for Simon96/96 with GA + Pipeline")
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simon9696')
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  Basic CNN+Transformer Distinguisher  Simon96/96")
