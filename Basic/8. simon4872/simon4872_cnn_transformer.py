"""
CNN + Transformer Basic Related-Key Differential Neural Distinguisher for Simon48/72
...
# 33 round constants: 36 rounds - 3 initial key words = 33 iterations.
_BASE = 0xFFFFFC
const_simon4872 = [(_BASE ^ _z0[i % 62]) & MASK_PY for i in range(33)]
assert len(const_simon4872) == 33
...
def expand_key_simon(k, t):
    """Key schedule for Simon48/72 (m=3). k: shape (3, n_samples)."""
    if t < 3:
...
def _build_one_diff_samples_basic(n, nr, diff, key_diff, s_groups=8, extra_words=2):
    ...
    """
    keys = np.frombuffer(urandom(4 * 3 * n), dtype=np.uint32).reshape(3, -1) & MASK_VAL
    keys_diff = np.array([
...
def _build_random_negative_samples(n, nr, s_groups=8, extra_words=2):
    ...
    Simon48/72 has m=3 -- three 24-bit key words (stored in uint32).
    """
    keys_a = (np.frombuffer(urandom(4 * 3 * n), dtype=np.uint32)
              .reshape(3, -1).copy() & MASK_VAL)
    keys_b = (np.frombuffer(urandom(4 * 3 * n), dtype=np.uint32)
              .reshape(3, -1).copy() & MASK_VAL)
...
def make_train_data_basic(n, nr, diff=(0x0, 0x100000),
                          key_diff=(0x0, 0x0, 0x100000),
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    dk = (0, 0, dp_r)   # 3‑tuple for Simon48/72 (m=3)

    keys = np.frombuffer(urandom(4 * 3 * n_samples), dtype=np.uint32).reshape(3, -1) & MASK_VAL
    keys_d = np.array([
        keys[0] ^ dk[0],
        keys[1] ^ dk[1],
        keys[2] ^ dk[2],
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simon48_72_Basic_v2')
...
# ============================================================================
# Table 4 defaults (basic distinguisher) for Simon48/72 – used as fallback
# ============================================================================

TABLE4_DIFFS = [
    {"nr": 11, "diff": (0x0, 0x100000), "key_diff": (0x0, 0x0, 0x100000),
     "expected_acc": 0.9926, "expected_tpr": 0.9990, "expected_tnr": 0.9863},
    {"nr": 12, "diff": (0x0, 0x100000), "key_diff": (0x0, 0x0, 0x100000),
     "expected_acc": 0.6565, "expected_tpr": 0.6669, "expected_tnr": 0.6460},
]
...
def train_basic_distinguisher(nr,
                              ...
                              se_ratio=4,
                              output_dir='./results_simon4872'):
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simon48/72  Basic CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simon48/72 Basic CNN+Transformer v2 -- {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simon48/72 Basic  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simon48/72 Basic  {nr}r  MSE Loss')
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE (Basic)  --  Simon48/72  --  {nr} rounds")
...
def check_simon4872():
    ...
    print(f"  check 4 PASSED: 11r diff sample  "
          f"dL={hex(int(dL[0]))}  dR={hex(int(dR[0]))}")
    print("  Simon48/72 cipher checks PASSED")


def quick_data_check(nr=11, n=200, extra_words=2):
    diff = (0x0, 0x100000)
    key_diff = (0x0, 0x0, 0x100000)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Basic Related-Key Neural Distinguisher for Simon48/72 with GA + Pipeline")
    parser.add_argument('--rounds', type=int, nargs='+', default=[11, 12],
                        help='Round number(s). Default: 11 12')
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simon4872')
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  Basic CNN+Transformer Distinguisher  Simon48/72")
