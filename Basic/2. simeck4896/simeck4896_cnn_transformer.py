"""
CNN + Transformer Basic Related-Key Differential Neural Distinguisher for Simeck48/96
...
─────────────────────────────────────────────
  ...
  8 groups × 8 words × 24 bits = 1536 bits per sample
...
# ============================================================================
# Simeck48/96 cipher (unchanged)
# ============================================================================

WORD_SIZE = 24
MASK_VAL = 0xFFFFFF

# Precomputed round constants for Simeck48/96 (36 rounds)
const_simeck = [
    0xfffffd, 0xfffffd, 0xfffffd, 0xfffffd, 0xfffffd,
    0xfffffc, 0xfffffc, 0xfffffc, 0xfffffd, 0xfffffd,
    0xfffffc, 0xfffffd, 0xfffffd, 0xfffffd, 0xfffffc,
    0xfffffd, 0xfffffc, 0xfffffd, 0xfffffc, 0xfffffc,
    0xfffffc, 0xfffffc, 0xfffffd, 0xfffffc, 0xfffffc,
    0xfffffd, 0xfffffc, 0xfffffd, 0xfffffd, 0xfffffc,
    0xfffffc, 0xfffffd, 0xfffffd, 0xfffffd, 0xfffffd,
    0xfffffd
]


def rol(x, k):
    """Rotate left by k bits (24‑bit word)."""
...
def expand_key_simeck(k, t):
    """Key schedule for Simeck48/96. k: shape (4, n_samples)."""
    if t < 4:
...
def _build_one_diff_samples_basic(n, nr, diff, key_diff, s_groups=8):
    ...
    """
    keys = np.frombuffer(urandom(16 * n), dtype=np.uint32).reshape(4, -1) & MASK_VAL
    keys_diff = np.array([
        keys[0] ^ key_diff[0],
        keys[1] ^ key_diff[1],
        keys[2] ^ key_diff[2],
...
def _build_negative_samples_basic(n, nr, s_groups=8):
    ...
    keys_a = np.frombuffer(urandom(16 * n), dtype=np.uint32).reshape(4, -1) & MASK_VAL
    keys_b = np.frombuffer(urandom(16 * n), dtype=np.uint32).reshape(4, -1) & MASK_VAL
    ...
    for _ in range(s_groups):
        ...
        p0l = np.frombuffer(urandom(4 * n), dtype=np.uint32) & MASK_VAL
        p0r = np.frombuffer(urandom(4 * n), dtype=np.uint32) & MASK_VAL
        p1l = np.frombuffer(urandom(4 * n), dtype=np.uint32) & MASK_VAL
        p1r = np.frombuffer(urandom(4 * n), dtype=np.uint32) & MASK_VAL
...
def make_train_data_basic(n, nr, diff=(0x0, 0x2000),
                          key_diff=(0x0, 0x0, 0x0, 0x2000),
                          s_groups=8):
    """
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    dk = (0, 0, 0, dp_r)   # 4‑tuple for Simeck48/96 (m=4)

    keys = np.frombuffer(urandom(4 * 4 * n_samples), dtype=np.uint32).reshape(4, -1) & MASK_VAL
    keys_d = np.array([
        keys[0] ^ dk[0],
        keys[1] ^ dk[1],
        keys[2] ^ dk[2],
...
def make_model(num_blocks=2,
    ...
    token_dim = num_blocks * word_size * 4  # 192 bits per token for Simeck48/96
...
# ============================================================================
# Table 3 defaults (basic distinguisher) for Simeck48/96 – used as fallback
# ============================================================================

TABLE3_DIFFS = [
    {"nr": 15, "diff": (0x0, 0x2000), "key_diff": (0x0, 0x0, 0x0, 0x2000), "expected_acc": 0.9362},
    {"nr": 16, "diff": (0x0, 0x2000), "key_diff": (0x0, 0x0, 0x0, 0x2000), "expected_acc": 0.7812},
    {"nr": 17, "diff": (0x0, 0x2000), "key_diff": (0x0, 0x0, 0x0, 0x2000), "expected_acc": 0.6256},
    {"nr": 18, "diff": (0x0, 0x2000), "key_diff": (0x0, 0x0, 0x0, 0x2000), "expected_acc": 0.5503},
    {"nr": 19, "diff": (0x0, 0x2000), "key_diff": (0x0, 0x0, 0x0, 0x2000), "expected_acc": 0.5158},
]
...
def train_basic_distinguisher(nr,
    ...
    print(f"\n{'='*70}")
    print(f"Training Basic CNN-Transformer Distinguisher — Simeck48/96, {nr} rounds")
    ...
    ax1.set_title(f'Simeck48/96 Basic {nr}r — Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simeck48/96 Basic {nr}r — Loss')
    ...
    with open(result_path, 'w') as f:
        f.write(f"Simeck48/96 Basic CNN-Transformer Distinguisher — {nr} rounds\n")
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE (Basic)  --  Simeck48/96  --  {nr} rounds")
...
def check_simeck4896():
    """
    Sanity-check the Simeck48/96 cipher implementation.
    ...
    np.random.seed(13)
    n = 500
    p0l = np.frombuffer(urandom(4 * n), dtype=np.uint32).astype(np.uint32) & MASK_VAL
    p0r = np.frombuffer(urandom(4 * n), dtype=np.uint32).astype(np.uint32) & MASK_VAL
    keys = np.frombuffer(urandom(4 * 4 * n), dtype=np.uint32).reshape(4, -1).astype(np.uint32) & MASK_VAL

    ks36 = expand_key_simeck(keys, 36)

    # Forward encryption
    c0, c1 = encrypt_simeck((p0l, p0r), ks36)
    ...
    print("✓ Simeck48/96 cipher checks PASSED.")


def quick_data_check(nr=18, n=1000):
    diff = (0x0, 0x2000)
    key_diff = (0x0, 0x0, 0x0, 0x2000)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Basic Related-Key Neural Distinguisher for Simeck48/96 with GA + Pipeline")
    ...
    for nr in args.rounds:
        ...
        if fallback is None:
            print(f"WARNING: round {nr} not in Table 3 for Simeck48/96, skipping.")
            continue
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  Basic CNN+Transformer Distinguisher  Simeck48/96")
