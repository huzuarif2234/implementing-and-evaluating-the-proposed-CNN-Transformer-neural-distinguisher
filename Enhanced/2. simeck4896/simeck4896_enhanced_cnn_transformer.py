...
for Simeck48/96  --  MULTI-DIFFERENTIAL (POLYHEDRAL) VERSION
═══════════════════════════════════════════════════════════════════════════
...
# Round constants for Simeck48/96 (36 rounds)
const_simeck = [
...
def expand_key_simeck(k, t):
    """Key schedule for Simeck48/96. k is shape (4, n_samples)."""
    ks = [0] * t
...
#        dk = (dp_l, dp_r, dp_l, dp_r)   [Simeck48/96, m=4]
...
#    where m=1 (a single set bit) and n = WORD_SIZE (24 for Simeck48/96).
...
def ga_select_differentials(nr,
    ...
    Finds effective (δP, δK) pairs for Simeck48/96 over `nr` rounds.
...
def make_train_data_enhanced(n, nr, pos_diffs, neg_diffs,
    ...
    """
    Build the ENHANCED (polyhedral) training set for Simeck48/96.
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simeck48_96_Enhanced_v2')
...
def train_distinguisher_enhanced(
        ...
        output_dir='./results_simeck4896_enhanced',
):
    """Train the ENHANCED (polyhedral) distinguisher for Simeck48/96."""
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simeck48/96  ENHANCED CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simeck48/96  CNN+Transformer v2 ENHANCED  --  {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simeck48/96 Enhanced  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'],     label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simeck48/96 Enhanced  {nr}r  BCE Loss')
...
def check_simeck4896():
    """Verify Simeck48/96 correctness."""
    np.random.seed(13)
    n    = 500
    p0l  = _rand_words(n)
    p0r  = _rand_words(n)
    ...
    ks18  = expand_key_simeck(keys,  18)
    ks18d = expand_key_simeck(keys2, 18)
    c0l18, c0r18 = encrypt_simeck((p0l, p0r), ks18)
    c1l18, c1r18 = encrypt_simeck(
        ((p0l ^ diff[0]) & MASK_VAL, (p0r ^ diff[1]) & MASK_VAL), ks18d)
    dL = (c0l18 ^ c1l18) & MASK_VAL
    dR = (c0r18 ^ c1r18) & MASK_VAL
    print(f"  check 4 PASSED: 18r related-key diff sample  "
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE  --  Simeck48/96  --  {nr} rounds")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            '  python simeck4896_enhanced_cnn_transformer.py --rounds 18 19 20\n\n'
            '  # GA pipeline (recommended for unknown rounds)\n'
            '  python simeck4896_enhanced_cnn_transformer.py --rounds 20 \\\n'
            '      --use_pipeline --threshold_acc 0.55 --max_iterations 3\n\n'
            '  # Custom diffs, single-shot\n'
            '  python simeck4896_enhanced_cnn_transformer.py --rounds 19 \\\n'
            '      --pos_diffs "0x0,0x2000" "0x0,0x4000" \\\n'
            '      --neg_diffs "0x0,0x100000" "0x0,0x400000" \\\n'
            '      --pos_key_diffs "0x0,0x0,0x0,0x2000" "0x0,0x0,0x0,0x4000" \\\n'
            '      --neg_key_diffs "0x0,0x0,0x0,0x100000" "0x0,0x0,0x0,0x400000"\n\n'
            '  python simeck4896_enhanced_cnn_transformer.py --sanity_only\n'
        ),
    ...
    parser.add_argument('--pos_key_diffs', type=str, nargs='+', default=None,
                        help='4 words per entry for Simeck48/96')
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simeck4896_enhanced')
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  CNN+Transformer v2 ENHANCED  Simeck48/96")
