...
for Simeck32/64  --  MULTI-DIFFERENTIAL (POLYHEDRAL) VERSION
═══════════════════════════════════════════════════════════════════════════
...
PIPELINE (matches flowchart):
  ...
  [1] Related-Key Attack Scenario       (K, K') and (P, P')
     │
     ▼
  [2] GA-Based Differential Selection   <-- NEW: genetic algorithm
...
TARGET  ──  Table 6  (enhanced distinguishers for Simeck32/64)
  Round  Input / Key Differences                    Acc    TPR    TNR
  ─────  ─────────────────────────────────────────  ─────  ─────  ─────
  12r    pos: (0x0,0x40)   & (0x0,0x80)             0.999  0.999  0.999
         neg: (0x0,0x1000) & (0x0,0x4000)
  13r    pos: (0x0,0x40)   & (0x0,0x80)             0.755  0.759  0.751
         neg: (0x0,0x1000) & (0x0,0x4000)

  (key_diff has 4 components for Simeck32/64)
...
──────────────────────────────────────────
  ...
  8 groups × 10 words × 16 bits = 1280 bits per sample
...
# Round constants for Simeck32/64 (32 rounds)
const_simeck = [
    0xFFFC,
    0xFFFC,
    0xFFFC,
...
def expand_key_simeck(k, t):
    """Key schedule for Simeck32/64. k is shape (4, n_samples)."""
    ks = [0] * t
...
def encrypt_simeck(p, ks):
    x = (
        ...
        else np.atleast_1d(np.array(p[0], dtype=np.uint32))
    )
    y = (
        p[1].copy()
        if isinstance(p[1], np.ndarray)
        else np.atleast_1d(np.array(p[1], dtype=np.uint32))
    )
    for k in ks:
...
def _build_one_diff_samples(n, nr, diff, key_diff, s_groups=8, extra_words=2):
    ...
    """
    keys = (
        np.frombuffer(urandom(2 * 4 * n), dtype=np.uint16)
        .reshape(4, -1)
        .astype(np.uint32)
        & MASK_VAL
    )
    ...
    for _ in range(s_groups):
        p0l = (
            np.frombuffer(urandom(2 * n), dtype=np.uint16).astype(np.uint32) & MASK_VAL
        )
        p0r = (
            np.frombuffer(urandom(2 * n), dtype=np.uint16).astype(np.uint32) & MASK_VAL
        )
...
#    Each chromosome encodes only the PLAINTEXT difference δP as two 16-bit
#    half-words [dp_l, dp_r].  The KEY difference δK is ALWAYS set equal to
#    δP (replicated across all four key words):
#        dk = (dp_l, dp_r, dp_l, dp_r)   [Simeck32/64, m=4]
...
#    where m=1 (a single set bit) and n = WORD_SIZE (16).
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    """
    dp_l = int(dp_l) & MASK_VAL
    dp_r = int(dp_r) & MASK_VAL

    # Paper constraint: δK identical to δP (replicated across m=4 key words)
    dk = (dp_l, dp_r, dp_l, dp_r)

    # Degenerate zero-difference is always trivially distinguishable — skip it
    if dp_l == 0 and dp_r == 0:
        return 0.0

    # Sample random keys and plaintexts
    keys = (
        np.frombuffer(urandom(2 * 4 * n_samples), dtype=np.uint16)
        .reshape(4, -1)
        .astype(np.uint32)
        & MASK_VAL
    )
    keys_d = np.array([keys[i] ^ dk[i] for i in range(4)], dtype=np.uint32) & MASK_VAL

    ks = expand_key_simeck(keys, nr)
    ks_d = expand_key_simeck(keys_d, nr)

    p0l = (
        np.frombuffer(urandom(2 * n_samples), dtype=np.uint16).astype(np.uint32)
        & MASK_VAL
    )
    p0r = (
        np.frombuffer(urandom(2 * n_samples), dtype=np.uint16).astype(np.uint32)
...
def _chrom_bias(chrom, nr, n_samples):
    """Wrapper: evaluate bias score for a chromosome [dp_l, dp_r]."""
    return _bias_score(int(chrom[0]), int(chrom[1]), nr, n_samples)
...
def _generate_initial_population(L_squared, plain_bits, min_hw, max_hw, rng):
    ...
    plain_bits : int  — total plaintext bit-width (32 for Simeck32/64)
    ...
    """
    half = plain_bits // 2  # 16 bits per half-word for Simeck32/64
    population = []
    attempts = 0
    max_attempts = L_squared * 200  # safety cap to avoid infinite loop

    while len(population) < L_squared and attempts < max_attempts:
        attempts += 1
        # Draw a random plaintext difference as a single `plain_bits`-wide integer
        val = int(rng.integers(0, 1 << plain_bits))
        hw = bin(val).count("1")
        if min_hw <= hw <= max_hw:
            dp_l = (val >> half) & MASK_VAL
            dp_r = val & MASK_VAL
            population.append([dp_l, dp_r])

    if len(population) < L_squared:
        # Fallback: pad with unconstrained random chromosomes
...
def ga_select_differentials(
    ...
    Finds effective (δP, δK) pairs for Simeck32/64 over `nr` rounds.
    ...
    plain_population = _generate_initial_population(
        pop_size, 2 * WORD_SIZE, min_hw, max_hw, rng
    )

    # key_population ← plain_population  (Algorithm 1, line 3)
    # (δK is derived on-the-fly in _bias_score; we only store δP)
    starting_bias = np.array([_chrom_bias(c, nr, n_fitness) for c in plain_population])
    ...
    if verbose:
        print(
            f"  Initial pool: {pop_size} → kept top {_PAPER_WORKING_POP}  "
    ...
    for gen in range(n_generations):
        ...
        for i in range(n_pop):
            for j in range(i + 1, n_pop):
                rand_bit = int(rng.integers(0, 2 * WORD_SIZE))  # 0..31
                half = WORD_SIZE

                # XOR the full 32-bit differences and add random bit flip
                base_l = int(current_population[i][0]) ^ int(current_population[j][0])
                base_r = int(current_population[i][1]) ^ int(current_population[j][1])

                if rand_bit < half:
                    # bit falls in the right half-word
                    flip_l = 0
                    flip_r = 1 << rand_bit
                else:
                    # bit falls in the left half-word
                    flip_l = 1 << (rand_bit - half)
                    flip_r = 0

                cand_l = (base_l ^ flip_l) & MASK_VAL
                cand_r = (base_r ^ flip_r) & MASK_VAL
                candidates.append([cand_l, cand_r])
        ...
        if verbose:
            print(
                f"  [GA]  iter {gen+1:>2}/{n_generations}   "
    ...
    for i in range(len(current_population)):
        ...
        if bs < threshold:
            continue
        dp_l = int(c[0]) & MASK_VAL
        dp_r = int(c[1]) & MASK_VAL
        ...
        if key in seen:
            continue
        seen.add(key)
        # δK = δP replicated across all four Simeck32/64 key words
        dk = (dp_l, dp_r, dp_l, dp_r)
        filtered_pool.append(((dp_l, dp_r), dk, bs))
    ...
    if verbose:
        ...
        print(
            f"  [GA]  0.06-close pool : {len(filtered_pool)} candidates "
            f"(threshold ≥ {threshold:.5f})"
        )
        print(f"  [GA]  Returning top-{min(n_select, len(filtered_pool))}:")
        for i, (dp, dk, bs) in enumerate(filtered_pool[:n_select]):
            print(
                f"    {i+1}. ΔP={tuple(hex(v) for v in dp)}  "
                f"ΔK={tuple(hex(v) for v in dk)}  bias={bs:.5f}"
            )
...
def make_train_data_enhanced(n, nr, pos_diffs, neg_diffs, s_groups=8, extra_words=2):
    """
    Build the ENHANCED (polyhedral) training set for Simeck32/64.
    ...
    for i, (diff, key_diff) in enumerate(pos_diffs):
        chunk_n = (
            n_pos // t_pos if i < t_pos - 1 else n_pos - (n_pos // t_pos) * (t_pos - 1)
        )
        pos_chunks.append(
            _build_one_diff_samples(
    ...
    for i, (diff, key_diff) in enumerate(neg_diffs):
        chunk_n = (
            n_neg // t_neg if i < t_neg - 1 else n_neg - (n_neg // t_neg) * (t_neg - 1)
        )
        neg_chunks.append(
            _build_one_diff_samples(
                chunk_n, nr, diff, key_diff, s_groups=s_groups, extra_words=extra_words
...
def transformer_encoder_block(x, num_heads, ff_dim, dropout_rate, reg_param, name="tr"):
    ...
    ffn = Dense(
        ff_dim, activation="relu", kernel_regularizer=l2(reg_param), name=f"{name}_ffn1"
    )(x)
    ffn = Dropout(dropout_rate, name=f"{name}_drop")(ffn)
    ffn = Dense(d_model, kernel_regularizer=l2(reg_param), name=f"{name}_ffn2")(ffn)
...
def make_model(
    ...
    for i in range(cnn_depth):
        ...
        c = se_block(
            c, num_filters, se_ratio=se_ratio, reg_param=reg_param, name=f"se{i}"
        )
    ...
    out = Dense(
        1, activation=final_activation, kernel_regularizer=l2(reg_param), name="output"
    )(x)

    return Model(
        inputs=inp, outputs=out, name="CNN_Transformer_Simeck32_64_Enhanced_v2"
    )
...
# ═══════════════════════════════════════════════════════════════════════════
# Table 6 targets  (enhanced distinguishers for Simeck32/64)
# ═══════════════════════════════════════════════════════════════════════════
...
TABLE6_SIMECK3264 = [
    {
        ...
        "neg_diffs": [
            ((0x0, 0x1000), (0x0, 0x0, 0x0, 0x1000)),
            ((0x0, 0x4000), (0x0, 0x0, 0x0, 0x4000)),
        ],
        "expected_acc": 0.999,
        "expected_tpr": 0.999,
        "expected_tnr": 0.999,
...
_TABLE6_DEFAULT = {e["nr"]: e for e in TABLE6_SIMECK3264}
...
def train_distinguisher_enhanced(
    ...
    num_transformer_blocks=3,
    se_ratio=4,
    output_dir="./results_simeck3264_enhanced",
):
    """Train the ENHANCED (polyhedral) distinguisher for Simeck32/64."""
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simeck32/64  ENHANCED CNN+Transformer v2  --  {nr} rounds")
    print(f"  POSITIVE diffs:")
    for d, k in pos_diffs:
        print(
            f"    diff={tuple(hex(v) for v in d)}  "
    ...
    train_ds = (
        tf.data.Dataset.from_tensor_slices((X_tr, Y_tr_f32))
        .shuffle(buffer_size=n_train, seed=42)
        .batch(batch_size)
        .prefetch(tf.data.AUTOTUNE)
    )
    val_ds = (
        tf.data.Dataset.from_tensor_slices((X_val, Y_val_f32))
        .batch(batch_size)
        .prefetch(tf.data.AUTOTUNE)
    )
    ...
    print(f"\n{'─'*64}")
    print(
        f"  {nr}r  ENHANCED  Best epoch={best_ep+1}  "
    ...
    with open(txt, "w") as f:
        f.write(f"Simeck32/64  CNN+Transformer v2 ENHANCED  --  {nr} rounds\n")
        f.write(
            f"filters={num_filters} depth={depth} d1={d1} d2={d2} "
            f"se_ratio={se_ratio} dropout={dropout_rate}\n"
        )
        f.write(
            f"transformer: {num_transformer_blocks}x heads={num_heads} "
            f"ff={ff_dim}\n"
        )
        ...
        f.write(
            f"data: {s_groups}g x {words_per_group}w x {WORD_SIZE}b = "
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f"Simeck32/64 Enhanced  {nr}r  Accuracy")
    ax1.set_xlabel("Epoch")
    ax1.set_ylabel("Accuracy")
    ax1.legend()
    ax2.plot(history.history["loss"], label="Train")
    ax2.plot(history.history["val_loss"], label="Val")
    ax2.set_title(f"Simeck32/64 Enhanced  {nr}r  BCE Loss")
...
def check_simeck3264():
    """Verify Simeck32/64 correctness."""
    np.random.seed(13)
    n = 500
    ...
    c0l, c0r = encrypt_simeck((p0l, p0r), ks32)
    ...
    print("  check 1 PASSED: encrypt/decrypt round-trip (500 samples, 32 rounds, m=4)")
    ...
    print("  check 3 PASSED: key schedules valid for 12r / 13r")
    ...
    ks12 = expand_key_simeck(keys, 12)
    ks12d = expand_key_simeck(keys2, 12)
    c0l12, c0r12 = encrypt_simeck((p0l, p0r), ks12)
    c1l12, c1r12 = encrypt_simeck(
        ((p0l ^ diff[0]) & MASK_VAL, (p0r ^ diff[1]) & MASK_VAL), ks12d
    )
    dL = (c0l12 ^ c1l12) & MASK_VAL
    dR = (c0r12 ^ c1r12) & MASK_VAL
    print(
        f"  check 4 PASSED: 12r Table6 diff sample  "
        f"dL={hex(int(dL[0]))}  dR={hex(int(dR[0]))}"
    )
    print("  Simeck32/64 cipher checks PASSED")


def quick_data_check_enhanced(nr=12, n=200, extra_words=2):
    pos_diffs = [
        ((0x0, 0x40), (0x0, 0x0, 0x0, 0x40)),
        ((0x0, 0x80), (0x0, 0x0, 0x0, 0x80)),
    ]
...
def run_full_pipeline(
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE  --  Simeck32/64  --  {nr} rounds")
    print(
    ...
    for it in range(1, max_iterations + 1):
        ...
        if use_ga:
            ...
            if best_bias > MIN_USEFUL_BIAS and len(ga_all) >= 2:
                ...
                print(f"  [Step 2]  ⚠ Falling back to Table 6 known-good differentials")
                if fallback_pos_diffs is None or fallback_neg_diffs is None:
                    raise ValueError(
                        "GA failed and no fallback_pos_diffs/neg_diffs provided!"
                    )
        ...
        if passed:
            ...
            print(
                f"     acc={res['acc']:.4f}  TPR={res['tpr']:.4f}  TNR={res['tnr']:.4f}"
            )
            print("🏆" * 38)
            print("\n  STOP\n")
        ...
        if it < max_iterations:
            ...
            print(
                f"     • Improve GA Search   pop {cur_ga_pop} -> {cur_ga_pop + 20}, "
                f"gens {cur_ga_generations} -> {cur_ga_generations + 5}  "
                f"(bias-score fitness / Algorithm 1)"
            )
            print(
                f"     • Enhance Dataset      n_train {cur_n_train:,} -> "
                f"{int(cur_n_train * 1.5):,}"
            )
            print(
                f"     • Adjust Architecture  depth {cur_depth} -> {cur_depth + 2}, "
    ...
    print("\n" + "─" * 76)
    print(
        f"  Best across all iterations: iter {best['iteration']}  "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for Simeck32/64  (Table 6 of the paper)\n"
            "\n"
            "Two modes:\n"
            "  (a) Single-shot (default): trains once with given/Table-6 diffs.\n"
            ...
            "\n"
            "Table 6 targets (Simeck32/64):\n"
            "  12r: pos=(0x0,0x40),(0x0,0x80)  neg=(0x0,0x1000),(0x0,0x4000)\n"
            "       acc=0.999  TPR=0.999  TNR=0.999\n"
            "  13r: same diffs  acc=0.755  TPR=0.759  TNR=0.751\n"
            "\n"
            "Examples:\n"
            "  # Single-shot, original behaviour\n"
            "  python simeck3264_enhanced_cnn_transformer.py --rounds 12 13\n\n"
            "  # Full closed-loop pipeline (Steps 1-10) for 12 rounds\n"
            "  python simeck3264_enhanced_cnn_transformer.py --rounds 12 \\\n"
            "      --use_pipeline --max_iterations 3\n\n"
            "  # Custom diffs, single-shot\n"
            "  python simeck3264_enhanced_cnn_transformer.py --rounds 13 \\\n"
            '      --pos_diffs "0x0,0x40" "0x0,0x80" \\\n'
            '      --neg_diffs "0x0,0x1000" "0x0,0x4000" \\\n'
            '      --pos_key_diffs "0x0,0x0,0x0,0x40" "0x0,0x0,0x0,0x80" \\\n'
            '      --neg_key_diffs "0x0,0x0,0x0,0x1000" "0x0,0x0,0x0,0x4000"\n\n'
            "  python simeck3264_enhanced_cnn_transformer.py --sanity_only\n"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )

    parser.add_argument("--rounds", type=int, nargs="+", default=[12, 13])
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="4 words per entry for Simeck32/64",
    )
    ...
    parser.add_argument(
        "--output_dir", type=str, default="./results_simeck3264_enhanced"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Accuracy threshold for Step 9. Defaults to Table 6 value "
        "for the round if available.",
    )
    ...
    have_manual = (
        args.pos_diffs is not None
        and args.neg_diffs is not None
    ...
    if have_manual:
        ...
        if len(cli_pos_plain) != len(cli_pos_key):
            raise ValueError(
                "--pos_diffs and --pos_key_diffs must have the same number of entries."
            )
        if len(cli_neg_plain) != len(cli_neg_key):
            raise ValueError(
                "--neg_diffs and --neg_key_diffs must have the same number of entries."
            )
    ...
    for nr in args.rounds:
        if cli_pos_diffs is not None:
            ...
            t6 = _TABLE6_DEFAULT.get(nr)
            expected_acc = t6["expected_acc"] if t6 else None
            expected_tpr = t6["expected_tpr"] if t6 else None
            expected_tnr = t6["expected_tnr"] if t6 else None
            ...
            print(
                f"  Supply all four --pos_diffs / --neg_diffs / "
        ...
        if args.use_pipeline:
            thr_acc = (
                args.threshold_acc if args.threshold_acc is not None else expected_acc
            )
            thr_tpr = (
                args.threshold_tpr if args.threshold_tpr is not None else expected_tpr
            )
            thr_tnr = (
                args.threshold_tnr if args.threshold_tnr is not None else expected_tnr
            )
            if thr_acc is None:
                print(
                    f"  No threshold available for round {nr} - "
    ...
    if all_results:
        print("\n" + "=" * 90)
        print(
            "  FINAL SUMMARY  --  CNN+Transformer v2 ENHANCED  Simeck32/64  (Table 6)"
        )
        print(
            f"  {'Nr':<5} {'Pos-diff0':<14} {'Neg-diff0':<14} "
            f"{'Acc':<8} {'TPR':<8} {'TNR':<8} "
