...
for Simon32/64  --  MULTI-DIFFERENTIAL (POLYHEDRAL) VERSION
═══════════════════════════════════════════════════════════════════════════
...
────────────────────
  ...
  Key schedule uses z0 sequence (period 62) and ror operations
...
# z0 sequence (period 62) — Simon32/64 uses j=0
...
_BASE = 0xFFFC
# Simon32/64 round constants: needed for t-4 expansions where t up to NUM_ROUNDS_FULL
const_simon3264 = [(_BASE ^ _z0[i % 62]) & MASK_VAL for i in range(NUM_ROUNDS_FULL - 4 + 1)]
...
def F_simon32(x):
    """Simon32 round function: F(x) = (S1(x) AND S8(x)) XOR S2(x)."""
    return (rol(x, 1) & rol(x, 8)) ^ rol(x, 2)


def enc_one_round_simon32(p, k):
    c1 = p[0]
    c0 = (F_simon32(p[0]) ^ p[1] ^ k) & MASK_VAL
    return (c0, c1)


def expand_key_simon3264(k, t):
    """
    Key schedule for Simon32/64.  m=4: four 16-bit key words.
    k: shape (4, n), dtype uint32 (values masked to 16 bits).
    ...
    for i in range(t - 4):
        ...
        ks[i + 4] = (const_simon3264[i] ^ ks[i] ^ tmp ^ ror(tmp, 1)) & MASK_VAL
    return ks


def encrypt_simon3264(p, ks):
    ...
    for k in ks:
        x, y = enc_one_round_simon32((x, y), k)
...
F_simeck = F_simon32
expand_key_simeck = expand_key_simon3264
encrypt_simeck = encrypt_simon3264
...
#        dk = (dp_l, dp_r, dp_l, dp_r)   [Simon32/64, m=4]
...
#    where m=1 (a single set bit) and n = WORD_SIZE (16 for Simon32/64).
...
def ga_select_differentials(nr,
    ...
    Finds effective (δP, δK) pairs for Simon32/64 over `nr` rounds.
...
def make_train_data_enhanced(n, nr, pos_diffs, neg_diffs,
    ...
    """
    Build the ENHANCED (polyhedral) training set for Simon32/64.
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simon32_64_Enhanced_v2')
...
def train_distinguisher_enhanced(
        ...
        output_dir='./results_simon3264_enhanced',
):
    """Train the ENHANCED (polyhedral) distinguisher for Simon32/64."""
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simon32/64  ENHANCED CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simon32/64  CNN+Transformer v2 ENHANCED  --  {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simon32/64 Enhanced  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'],     label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simon32/64 Enhanced  {nr}r  BCE Loss')
...
def check_simon3264():
    ...
    ks_full     = expand_key_simon3264(keys, NUM_ROUNDS_FULL)
    c0l, c0r    = encrypt_simon3264((p0l, p0r), ks_full)
    rx, ry      = c0l.copy(), c0r.copy()
    for k in reversed(ks_full):
        rx, ry = ry, (F_simon32(ry) ^ rx ^ k) & MASK_VAL
    ...
    for t in [11, 12, 13]:
        kst = expand_key_simon3264(keys, t)
        assert len(kst) == t
    ...
    ks12  = expand_key_simon3264(keys,  12)
    ks12d = expand_key_simon3264(keys2, 12)
    c0l12, c0r12 = encrypt_simon3264((p0l, p0r), ks12)
    c1l12, c1r12 = encrypt_simon3264(
        ((p0l ^ diff[0]) & MASK_VAL, (p0r ^ diff[1]) & MASK_VAL), ks12d)
    dL = (c0l12 ^ c1l12) & MASK_VAL
    dR = (c0r12 ^ c1r12) & MASK_VAL
    print(f"  check 4 PASSED: 12r related-key diff sample  "
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE  --  Simon32/64  --  {nr} rounds")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            '  python simon3264_enhanced_cnn_transformer.py --rounds 11 12 13\n\n'
            '  # GA pipeline (recommended for unknown rounds)\n'
            '  python simon3264_enhanced_cnn_transformer.py --rounds 13 \\\n'
            '      --use_pipeline --threshold_acc 0.55 --max_iterations 3\n\n'
            '  # Custom diffs, single-shot\n'
            '  python simon3264_enhanced_cnn_transformer.py --rounds 12 \\\n'
            ...
            '  python simon3264_enhanced_cnn_transformer.py --sanity_only\n'
        ),
    ...
    parser.add_argument('--pos_key_diffs', type=str, nargs='+', default=None,
                        help='4 words per entry for Simon32/64')
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simon3264_enhanced')
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  CNN+Transformer v2 ENHANCED  Simon32/64")
