"""
CNN + Transformer Basic Related-Key Differential Neural Distinguisher for Simon64/128
...
# ============================================================================
# Simon64/128 cipher (unchanged)
# ============================================================================

WORD_SIZE = 32
MASK_VAL  = 0xFFFFFFFF

# 40 round constants for Simon64/128 (44 rounds -> 40 constants after first 4)
const_simon64 = [
    0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffd,
    0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffd,
    0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffc,
    0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffc,
    0xfffffffc, 0xfffffffd, 0xfffffffd, 0xfffffffc,
    0xfffffffc, 0xfffffffd, 0xfffffffc, 0xfffffffd,
    0xfffffffd, 0xfffffffd, 0xfffffffd, 0xfffffffc,
    0xfffffffc, 0xfffffffc, 0xfffffffc, 0xfffffffc,
    0xfffffffc, 0xfffffffd, 0xfffffffc, 0xfffffffc,
    0xfffffffd, 0xfffffffc, 0xfffffffc, 0xfffffffc,
]
assert len(const_simon64) == 40
...
def expand_key_simon(k, t):
    """Key schedule for Simon64/128. k: shape (4, n_samples)."""
    if t < 4:
    ...
    for i in range(t - 4):
        ...
        ks[i + 4] = (const_simon64[i] ^ ks[i] ^ tmp ^ ror(tmp, 1)) & MASK_VAL
    return ks
...
def _build_random_negative_samples(n, nr, s_groups=8, extra_words=2):
    ...
    Simon64/128 has m=4 -- four 32-bit key words.
    """
...
def make_train_data_basic(n, nr, diff=(0x0, 0x1000),
                          key_diff=(0x0, 0x0, 0x0, 0x1000),
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    dk = (0, 0, 0, dp_r)   # 4‑tuple for Simon64/128 (m=4)

    keys = np.frombuffer(urandom(16 * n_samples), dtype=np.uint32).reshape(4, -1)
    keys_d = np.array([
        keys[0] ^ dk[0],
        keys[1] ^ dk[1],
        keys[2] ^ dk[2],
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simon64_128_Basic_v2')
...
# ============================================================================
# Table 3 defaults (basic distinguisher) for Simon64/128 – used as fallback
# ============================================================================

TABLE3_DIFFS = [
    {"nr": 13, "diff": (0x0, 0x1000), "key_diff": (0x0, 0x0, 0x0, 0x1000),
     "expected_acc": 0.8399, "expected_tpr": 0.8412, "expected_tnr": 0.8467},
    {"nr": 14, "diff": (0x0, 0x1000), "key_diff": (0x0, 0x0, 0x0, 0x1000),
     "expected_acc": 0.5789, "expected_tpr": 0.5788, "expected_tnr": 0.5790},
]
...
def train_basic_distinguisher(nr,
                              ...
                              se_ratio=4,
                              output_dir='./results_simon64128'):
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simon64/128  Basic CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simon64/128 Basic CNN+Transformer v2 -- {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simon64/128 Basic  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simon64/128 Basic  {nr}r  MSE Loss')
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE (Basic)  --  Simon64/128  --  {nr} rounds")
...
def check_simon64128():
    """Verify Simon64/128 against official test vector."""
    ...
    ks44 = expand_key_simon(keys, 44)
    c0l, c0r = encrypt_simon((p0l, p0r), ks44)
    ...
    print("  check 2 PASSED: encrypt/decrypt round-trip (500 samples, 44 rounds, m=4)")

    assert len(ks44) == 44
    assert all(np.all((k & MASK_VAL) == k) for k in ks44)
    print("  check 3 PASSED: 44 valid 32-bit round keys")

    for t in [13, 14]:
        kst = expand_key_simon(keys, t)
        assert len(kst) == t
    print("  check 4 PASSED: key schedules valid for 13r / 14r")
    print("  Simon64/128 cipher checks PASSED")


def quick_data_check(nr=13, n=400, extra_words=2):
    diff = (0x0, 0x1000)
    key_diff = (0x0, 0x0, 0x0, 0x1000)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Basic Related-Key Neural Distinguisher for Simon64/128 with GA + Pipeline")
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simon64128')
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  Basic CNN+Transformer Distinguisher  Simon64/128")
