"""
CNN + Transformer Basic Related-Key Differential Neural Distinguisher for Simon32/64
...
def expand_key_simon(k, t):
    """Key schedule for Simon32/64. k: shape (4, n_samples)."""
    if t < 4:
    ...
    for i in range(t - 4):
        ...
        ks[i + 4] = (const_simon32[i] ^ ks[i] ^ tmp ^ ror(tmp, 1)) & MASK_VAL
    return ks


def encrypt_simon(p, ks):
    x = p[0].copy() if isinstance(p[0], np.ndarray) else np.atleast_1d(
            np.array(p[0], dtype=np.uint16))
    y = p[1].copy() if isinstance(p[1], np.ndarray) else np.atleast_1d(
            np.array(p[1], dtype=np.uint16))
    for k in ks:
        x, y = enc_one_round_simon((x, y), k)
    return (x.astype(np.uint16) & MASK_VAL, y.astype(np.uint16) & MASK_VAL)
...
def _build_one_diff_samples_basic(n, nr, diff, key_diff, s_groups=8, extra_words=2):
    ...
    for _ in range(s_groups):
        ...
        p1l = (p0l ^ np.uint16(diff[0])) & MASK_VAL
        p1r = (p0r ^ np.uint16(diff[1])) & MASK_VAL
...
def _build_random_negative_samples(n, nr, s_groups=8, extra_words=2):
    ...
    Simon32/64 has m=4 -- four 16-bit key words.
    """
    keys_a = np.frombuffer(urandom(8 * n), dtype=np.uint16).reshape(4, -1).copy()
    keys_b = np.frombuffer(urandom(8 * n), dtype=np.uint16).reshape(4, -1).copy()
    ...
    for _ in range(s_groups):
        p0l = np.frombuffer(urandom(2 * n), dtype=np.uint16).copy()
        p0r = np.frombuffer(urandom(2 * n), dtype=np.uint16).copy()
        p1l = np.frombuffer(urandom(2 * n), dtype=np.uint16).copy()
        p1r = np.frombuffer(urandom(2 * n), dtype=np.uint16).copy()
...
def make_train_data_basic(n, nr, diff=(0x0, 0x8010),
                          key_diff=(0x0, 0x0, 0x0, 0x8010),
...
def _bias_score(dp_l, dp_r, nr, n_samples):
    ...
    dk = (0, 0, 0, dp_r)   # 4‑tuple for Simon32/64 (m=4)

    keys = np.frombuffer(urandom(2 * 4 * n_samples), dtype=np.uint16).reshape(4, -1)
    keys_d = np.array([
        keys[0] ^ dk[0],
        keys[1] ^ dk[1],
        keys[2] ^ dk[2],
    ...
    p0l = np.frombuffer(urandom(2 * n_samples), dtype=np.uint16)
    p0r = np.frombuffer(urandom(2 * n_samples), dtype=np.uint16)
    p1l = p0l ^ dp_l
    p1r = p0r ^ dp_r
...
def _generate_initial_population(L_squared, plain_bits, min_hw, max_hw, rng):
    ...
    if len(population) < L_squared:
        while len(population) < L_squared:
            dp_r = int(rng.integers(1, MASK_VAL + 1))
            population.append([dp_r])
    return np.array(population, dtype=np.uint16)
...
def ga_select_differentials(nr,
    ...
    for gen in range(n_generations):
        ...
        candidates = np.array(candidates, dtype=np.uint16)
...
def make_model(s_groups=8,
    ...
    return Model(inputs=inp, outputs=out,
                 name='CNN_Transformer_Simon32_64_Basic_v2')
...
# ============================================================================
# Table 3 defaults (basic distinguisher) for Simon32/64 – used as fallback
# ============================================================================

TABLE3_DIFFS = [
    {"nr": 12, "diff": (0x0, 0x8010), "key_diff": (0x0, 0x0, 0x0, 0x8010),
     "expected_acc": 0.6766, "expected_tpr": 0.6793, "expected_tnr": 0.6738},
    {"nr": 13, "diff": (0x0, 0x8010), "key_diff": (0x0, 0x0, 0x0, 0x8010),
     "expected_acc": 0.5444, "expected_tpr": 0.5145, "expected_tnr": 0.5742},
    {"nr": 12, "diff": (0x0, 0x40), "key_diff": (0x0, 0x0, 0x0, 0x40),
     "expected_acc": 0.6477, "expected_tpr": 0.6518, "expected_tnr": 0.6435},
    {"nr": 13, "diff": (0x0, 0x40), "key_diff": (0x0, 0x0, 0x0, 0x40),
     "expected_acc": 0.5262, "expected_tpr": 0.5437, "expected_tnr": 0.5081},
]
_TABLE3_DEFAULT = {e['nr']: e for e in TABLE3_DIFFS}  # will store only the first for each round (0x8010 by default)
...
def train_basic_distinguisher(nr,
                              ...
                              se_ratio=4,
                              output_dir='./results_simon3264'):
    os.makedirs(output_dir, exist_ok=True)
    words_per_group = 8 + extra_words

    print(f"\n{'='*76}")
    print(f"  Simon32/64  Basic CNN+Transformer v2  --  {nr} rounds")
    ...
    with open(txt, 'w') as f:
        f.write(f"Simon32/64 Basic CNN+Transformer v2 -- {nr} rounds\n")
...
def _plot_history(history, nr, diff_tag, output_dir, expected_acc=None):
    ...
    ax1.set_title(f'Simon32/64 Basic  {nr}r  Accuracy')
    ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy'); ax1.legend()
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_title(f'Simon32/64 Basic  {nr}r  MSE Loss')
...
def run_full_pipeline(nr,
    ...
    """
    print("\n" + "█" * 76)
    print(f"  FULL PIPELINE (Basic)  --  Simon32/64  --  {nr} rounds")
...
def check_simon3264():
    plain = (np.array([0x6565], dtype=np.uint16),
             np.array([0x6877], dtype=np.uint16))
    cipher = (0xc69b, 0xe9bb)
    keys = np.array([[0x1918],[0x1110],[0x0908],[0x0100]], dtype=np.uint16)
    ks = expand_key_simon(keys, 32)
    ...
    p0l = np.frombuffer(urandom(2 * n), dtype=np.uint16).copy()
    p0r = np.frombuffer(urandom(2 * n), dtype=np.uint16).copy()
    keys = np.frombuffer(urandom(8 * n), dtype=np.uint16).reshape(4, -1).copy()
    ks32 = expand_key_simon(keys, 32)
    c0l, c0r = encrypt_simon((p0l, p0r), ks32)
    ...
    print("  check 2 PASSED: encrypt/decrypt round-trip (500 samples, 32 rounds, m=4)")

    assert len(ks32) == 32
    assert all(np.all((k & MASK_VAL) == k) for k in ks32)
    print("  check 3 PASSED: 32 valid 16-bit round keys")

    for t in [12, 13]:
        kst = expand_key_simon(keys, t)
        assert len(kst) == t
    print("  check 4 PASSED: key schedules valid for 12r / 13r")
    print("  Simon32/64 cipher checks PASSED")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Basic Related-Key Neural Distinguisher for Simon32/64 with GA + Pipeline")
    parser.add_argument('--rounds', type=int, nargs='+', default=[12, 13],
                        help='Round number(s). Default: 12 13')
    ...
    parser.add_argument('--output_dir',  type=str,  default='./results_simon3264')
    ...
    # Build fallback map (use the 0x8010 diff for each round, as that is the main Table 3 entry)
    fallback_map = {}
    for e in TABLE3_DIFFS:
        if e['nr'] not in fallback_map:
            fallback_map[e['nr']] = e
    ...
    for nr in args.rounds:
        ...
        if fallback is None:
            print(f"WARNING: round {nr} not in Table 3, skipping.")
            continue
    ...
    if all_results:
        print("\n" + "=" * 90)
        print("  FINAL SUMMARY  --  Basic CNN+Transformer Distinguisher  Simon32/64")
